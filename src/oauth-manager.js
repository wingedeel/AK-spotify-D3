...
var OAuthManager = (function() {
  ...
  function obtainToken(options) {
    console.log('OAuthManager obtainToken')
    options = options || {};
    ...
    var promise = new Promise(function(resolve, reject) {
      ...
      // Message is received from oath-send-back script
      ...
      // Add event listener to this window to receieve 
      // a message (which will be the Spotify access token)
      ...
      // Open a window which take you to Spotify's authorisation page
      authWindow = window.open(
                               'https://accounts.spotify.com/authorize?' + toQueryString(params),
                               'Spotify',
                               'menubar=no,location=no,resizable=no,scrollbars=no,status=no, width=' + width + ', height=' + height + ', top=' + top + ', left=' + left
                               );
      // Create a setInterval to see when the authorisation window
      // is closed.
